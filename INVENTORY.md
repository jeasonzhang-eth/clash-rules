...
| MyProxy | `Custom/MyProxy.yaml` | 52 |   - DOMAIN-SUFFIX |   # 个人·强制走代理（最高优先级，可覆盖 AdBlock 误拦）。 |
| MyDirect | `Custom/MyDirect.yaml` | 14 |   - DOMAIN-SUFFIX |   # 个人·强制直连（最高优先级，可覆盖 AdBlock 误拦）。 |
| appleai | `Custom/appleai.yaml` | 6 |   - DOMAIN/  - DOMAIN-SUFFIX |   # > AppleAI |

**合计域名/规则条目：20074**（去重前，按文件累加）
